...
function getTemplate(title, msg, value, buttons, css) {
...
function showDialog(title, msg, value, buttons, options) {
  ...
  const innerHTML = getTemplate(title, msg, value, buttons, css);
  ...
  find(dialog, ['ok', 'input']).forEach(el => el.focus());
...
function tab(dialog, names) {
  ...
  find(dialog, [name]).forEach(el => el.focus());
}
...
function closeDialog(el, dialog, ok, cancel) {
  ...
  const value = find(dialog, ['input']).reduce((value, el) => el.value, null);
...
function find(element, names) {
  ...
  const elements = names
    .map(name => element.querySelector(`[data-name="js-${name}"]`))
    .filter(notEmpty);
...
function addListenerAll(event, parent, elements, fn) {
  find(parent, elements).forEach(el => el.addEventListener(event, fn));
}
