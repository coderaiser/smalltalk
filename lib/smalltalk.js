...
exports.select = (title, msg, value = '', options) => {
    if (!options) {
        options = {};
    }
    const {
    ...
    for (const option of selectOptions) {
        optionsHTML += `<option ${val == option.value.replace(/"/g, '&quot;') ? 'selected' : ''} value="${option.value}">${option.name}</option>`;
    }
    
    const valueStr = `<select ${multiple ? 'multiple' : ''} data-name="js-input">${optionsHTML}</select>`;
    const buttons = getButtons(options) || BUTTON_OK_CANCEL;
    
    return showDialog(title, msg, valueStr, buttons, options, true);
};
...
function closeDialog(el, dialog, ok, cancel) {
    ...
    if (input) {
        if (input.nodeName == 'INPUT') {
            ...
            value = find(dialog, ['input']).reduce((_value, el) => el.value, null);
        }
    }
