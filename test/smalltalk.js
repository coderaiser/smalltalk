...
test('smalltalk: alert: click', t => {
  ...
  smalltalk.alert('title', 'message');
  t.equal(
    ok.addEventListener.args.pop()[0],
    'click',
    'should set click listener'
  );
...
test('smalltalk: alert: close: querySelector', t => {
  ...
  t.equal(
    querySelector.args.pop().pop(),
    '.smalltalk',
    'should find smalltalk'
  );
  t.end();
});
...
test('smalltalk: confirm: click on close', t => {
  ...
  smalltalk.confirm('title', 'message').catch(() => {
    after();
...
test('smalltalk: prompt: click on ok', t => {
  ...
  smalltalk.prompt('title', 'message', value).then(result => {
    after();
...
test('smalltalk: prompt: click on cancel', t => {
  ...
  smalltalk.prompt('title', 'message', value).catch(() => {
    after();
    t.pass('should reject');
    t.end();
  });
...
test('smalltalk: prompt: click on cancel: cancel false', t => {
  ...
  smalltalk
    .prompt('title', 'message', value, { cancel: false })
...
function getCreateElement(el = {}) {
  ...
  if (!el.querySelector) el.querySelector = querySelector;

  if (!el.addEventListener) el.addEventListener = addEventListener;
